...
class CyberState extends RuntimeState {
    activeThreatVector: string = "";
    simulationStatus: string = "IDLE";
}
...
async function execWithRetry<T>(fn: () => Promise<T>, maxRetries = 3, initialDelayMs = 1000): Promise<T> {
    let attempt = 0;
...
class RedSentinelInferenceRunner implements InferenceRunner {
    ...
    async run(request: InferenceInput): Promise<InferenceOutput> {
        ...
        const promptText: string = lastItem?.content?.text || "";
        ...
        if (apiKey && this.genAI) {
            try {
                ...
                if (responseText) {
                    const modelMessage = ModelMessageItem.rehydrate({ text: responseText });
                    return {
                        items: [modelMessage],
        ...
        const modelMessage = ModelMessageItem.rehydrate({ text: generatedResponse });
        return {
            items: [modelMessage],
            tokenUsage: undefined,
            rowResponse: null
...
const { initializeRuntime, join, sendMessage, runLoop, resolveRuntime } = defineRuntime<CyberState>();
...
class WhenThreatDetected extends SituationSpecification {
    isSatisfiedBy({ event, participant }: SituationContext): boolean {
        return event.type === 'message.sent' && event.producerId !== participant.getId();
    }
}

class WhenProducerAnswered extends SituationSpecification {
    private targetProducerId: string;
    constructor(targetProducerId: string) {
        super();
...
const redHandler: SituationHandler = {
    specification: new WhenThreatDetected(),
    processor: {
        apply({ event, participant }) {
            const threatVector = typeof event.payload === 'string' 
                ? event.payload 
                : (event.payload as any)?.message || resolveRuntime().state.activeThreatVector || "Unknown threat vector";
...
const blueHandler: SituationHandler = {
    specification: new WhenProducerAnswered(redAgent.getId()),
    processor: {
        apply({ event, participant }) {
            const redPayload = event.payload as any;
...
const auditorHandler: SituationHandler = {
    specification: new WhenProducerAnswered(blueAgent.getId()),
    processor: {
        apply({ event, participant }) {
            const bluePayload = event.payload as any;
...
const transcriptObserverHandler: SituationHandler = {
...
const humanUser = createHuman({ 
    name: 'Security Analyst',
    capabilities: [],
    handlers: []
});

const transcriptObserver = createHuman({
